...
### Are you in the **mood**, because I certainly am. Here is how TikTok plays with your **Emotions**
...
So how "positive" and intense are the songs in the corpus? We can gauge this by using the Valence and Energy values. And what is more, we can categorise songs by emotions using these two values as well. This idea comes from the **valence-arousal model of emotion** developed by J. Russell, but I won't bother you with the details. All that you need to know, is that the model suggests that emotions are distributed along these two dimensions. The emotions are noted in the plot.

Whenever I am endlessly scrolling on TikTok, I always get a good laugh out of it or I get really pumped up by all the good vibes from the videos. Hence, I'd assumed that the distribution coverage from large to small of the emotion would be Happy, Calm, Sad, and Angry. And as expected, **the amount of Happy songs is the largest**. Contrasting, I am surprised about Angry being second place. Maybe the Spotify API's valence does not exactly line up with the valence as described by the model.

Hovering over the plot, we can identify the name of tracks. In the Sad quadrant we see a song with a very low Energy, *Steven Universe*. As we'd indicated, this is **one of our atypical songs** in the corpus. Similarirty, in the Calm quadrant, we have *Monkeys spinning Monkeys* which we'd also indicated as an atypical song. I got to admit this song is something else. You know, when you have a favourite song and you play it **over and over**. Well, with this specific song, you don't want to do that. At first the song can be quite calming indeed, but to listen to the entire 2:05 minutes? It will drive you insane and the song would better fit in the Angry quadrant.

Focusing on the different trends, we can see that the majority of Happy sons are Dance songs. Sad songs fall more often in the Other trend. Lastly, Angry and Calm songs have about a 1:1 distribution regarding trends. 

And you might wonder, why are some points in the plot larger then others? Good quesiton. The **size indicates the danceability** of the song. A quick glance shows us that the sizes aren't that much different between the two trends. 
...
#   geom_segment(aes(x = 63, xend = 63, y=1, yend= 127), col = 'red') + 
#   geom_segment(aes(x = 78, xend = 78, y=1, yend= 127), col = 'red') + 
#   annotate("text", 70.5, 7, label = "video", color = "white", size = 3, fontface="bold")+ 
#   geom_segment(aes(x = 25, xend = 25, y=1, yend= 127), col = 'white') + 
#   geom_segment(aes(x = 51, xend = 51, y=1, yend= 127), col = 'white') + 
#   annotate("text", 12.5, 7, label = "Rosalia", color = "white", size = 3, fontface="bold")+ 
#   annotate("text", 40.5, 7, label = "Drums", color = "white", size = 3, fontface="bold") + 
#   geom_segment(aes(x = 56, xend = 56, y=1, yend= 127), col = 'white') + 
#   geom_segment(aes(x = 66, xend = 66, y=1, yend= 127), col = 'white') + 
#   geom_segment(aes(x = 81, xend = 81, y=1, yend= 127), col = 'white') + 
#   geom_segment(aes(x = 96, xend = 96, y=1, yend= 127), col = 'white') 
# ssm_timbre <- TKN %>%
...
### Okay but what about those 15 seconds, are they any different than the rest of the song?
...
lemon_ssm_pitch <- lemonade %>%
  ...
  scale_fill_viridis_c(guide = "none") +
  theme_classic() +
  labs(x = "", y = "", title = "Lemonade Pitches") +
  # annotate("text", 27.5, 7, label = "Video", color = "white", size = 3, fontface="bold") + 
  geom_segment(aes(x = 24, xend = 24, y=1, yend= 192), col = 'white') +
  geom_segment(aes(x = 31, xend = 31, y=1, yend= 192), col = 'white') + 
  annotate("segment", x = 55, xend = 28, y = 50, yend = 50, colour = 'white',
...
wipe_ssm_pitch <- TKN %>%
  ...
  scale_fill_viridis_c(guide = "none") +
  theme_classic() +
  labs(x = "", y = "", title = "#WIPEITDOWN Pitches") +
  # annotate("text", 7.5, 7, label = "Video", color = "white", size = 3, fontface="bold") + 
...
Now that we've looked at the emotions of the songs, we are going to explore the timbre of two different songs.  

I've chosen the tracks *Lemonade* by Internet Money and *#WIPEITDOWN* by BMW KENNY. I picked these two songs as they had similar values for Spotify's features. The *Lemonade* track is associated with a Dance trend, whereas the *#WIPEITDOWN* is associated with Other trend. 

As we all might know, TikTok consists of many short videos typically of 15 seconds. What I wanted to find out with these plots, if those specific ±15 seconds are special within a track. For this we'll dive a bit deeper into these Self-similarity matices (SSMs).
...
### Oh that's a **classic** for sure... or is it really?
...
Here we have a keygram of the song *Eine kleine Nachtmusik K.* - Wolfgang Amadeus Mozart, which is an outlier in the corpus in the sense that it is the only classical piece in the corpus. 
