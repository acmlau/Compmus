...
```{r}
corpus %>%
  ggplot(aes(x = trends, y = track.duration_ms/1000, fill = trends))+
  scale_fill_manual(values = c("#9FBEE1", "#1E3D60")) + 
  geom_violin() + geom_point(size = 2, aes(colour = loudness)) + 
  scale_colour_distiller(palette = "PuRd") + 
  guides(fill = FALSE) + 
  labs(x = "Trend", y = "Duration (s)", title = "Violinplots to look into the duration and loudness", colour = "Loudness (dB)") + theme_light() +
  scale_y_continuous(breaks= seq(60,360, 60),minor_breaks = NULL)
  
```

***

Here follows a analysis of the plots.
